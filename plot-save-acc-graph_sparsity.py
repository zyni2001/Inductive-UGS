    ...
    for root, dirs, files in os.walk(input_directory):
        #for dir in dirs:
            for filename in files:
                if filename.endswith('DD_DiffPool_eval.log'):
                    ...
                    ax.plot(graph_sparcity, final_test_acc_list, linestyle='-', linewidth=3, label=f'{filename}')
            ax.set_xlabel('Graph Sparsity (%)', fontsize=22)

    # save graph sparcity and final test acc list to pkl
    with open('graph_sparsity_DD_new.pkl', 'wb') as f:
        pickle.dump(graph_sparcity, f)
    with open('final_test_acc_list_DD_new.pkl', 'wb') as f:
        pickle.dump(final_test_acc_list, f)
...
def initialize_args():
    ...
    args.dataset = "DD"
    args.root_directory = "/home/zhiyu/Inductive-UGS/TUDataset/log/"
    args.output_directory = "/home/zhiyu/Inductive-UGS/TUDataset/log/"
